...
- **LiveATC rejects non-browser user agents.** Every `.pls` resolve and stream fetch from the main process must send a browser-like `User-Agent`, and resolved URLs must be cached (resolve only on connect/reconnect) — never hammer with a bare/bot UA. Shared HTTP plumbing lives in `src/main/http.ts` (`browserUserAgent` + `httpGet`); both `plsResolver.ts` and `liveatcDirectory.ts` use it.
- **The LiveATC search page is behind Cloudflare — only Chromium's own network stack gets through.** `https://www.liveatc.net/search/?icao=<code>` (the channel-manager directory) returns a Cloudflare "you have been blocked" page to a plain `curl`/Node fetch even WITH a browser UA; the same request via Electron's `net.request` (Chromium TLS fingerprint) succeeds (verified 2026-07-19). Fetch it main-side through `http.ts`, never from a bare client, and only on an operator gesture (never on a timer). When it fails anyway, the KOSH airport falls back to the bundled snapshot in `src/shared/koshFallback.json` (see `koshFallback.ts`).
